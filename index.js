...
// Recore By 𝐌𝐚𝐨𝐮 𝐀𝐫𝐟𝐢𝐧〆ᴀᴄᴍ || Andrea
...
${chalk.green.bold("NAHIDA BOT-MD")}         
VERSION 1.00
BOT BY ${chalk.green.bold("𝐌𝐚𝐨𝐮 𝐀𝐫𝐟𝐢𝐧〆ᴀᴄᴍ")}
...
require("./nahida")(haikal, m, chatUpdate, store)
} catch (err) {
console.log(err)
}
})
...
//=================================================//
    haikal.ev.on('group-participants.update', async (grupp) => {
console.log(grupp)
      try {
        let metadata = await haikal.groupMetadata(grupp.id);
        let participants = grupp.participants;
          ...
          let nameUser = await haikal.getName(num)
          ...
          let lefts = `https://api.popcat.xyz/welcomecard?background=${bg}&text1=GOODBYE&text2=+${nameUser}&text3=Member+${membr}&avatar=${ppuser2}`
            if ( grupp.action === 'add' ) {
              await haikal.sendMessage(grupp.id, { image: { url: wlc }, caption: `✧━━━━━[ *WELCOME* ]━━━━━✧\n\nHello @${num.split("@")[0]} Welcome To 
*${metadata.subject}*
...
│ *Status    :* 
╰════────────•
 ©𝐌𝐚𝐨𝐮 𝐀𝐫𝐟𝐢𝐧〆ᴀᴄᴍ`, mentions: [num] })
            } else if ( grupp.action === 'remove' ) {
              await haikal.sendMessage(grupp.id, { image: { url: lefts }, caption: `✧━━━━━[ *GOOD BYE* ]━━━━━✧\n\nGoodbye @${num.split("@")[0]} I Hope You Don't Come Back`, mentions: [num]}) 
            } else if ( grupp.action === 'promote' ) {
              haikal.sendMessage(grupp.id, { mentions: [num], text: `@${num.split("@")[0]} Congratulations, Now you are a Group Admin` })
            } else if ( grupp.action === 'demote' ) {
              haikal.sendMessage(grupp.id, { mentions: [num], text: `@${num.split("@")[0]} Congratulations, You are in demote` })
            }
          }
      } catch (err) {
      console.log(err)
...
connectToWhatsApp();
    ...
    console.log(chalk.green.bold("[ ••• ] Succes Connected To You Whatsapp Number"))
}
